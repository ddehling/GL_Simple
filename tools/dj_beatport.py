...
Usage:
    ...
    python tools/dj_beatport.py login --paste          # paste token JSON
    python tools/dj_beatport.py login --pkce            # loopback OAuth (needs
                                                        # BEATPORT_CLIENT_ID)

    # 2. Discover + fit:
    python tools/dj_beatport.py search "melodic house" --bpm 118-126
    python tools/dj_beatport.py search "lane 8" --fit    # + fit vs your library
    python tools/dj_beatport.py fit 12345678             # deep fit one track id

    # 3. Buy-list:
    python tools/dj_beatport.py wish add 12345678
    python tools/dj_beatport.py wish list
    python tools/dj_beatport.py wish open                # open all in browser
"""
import argparse
import os
import sys
...
def _client():
    c = BP.BeatportClient()
    if not c.available():
        print("Not authenticated. Run:  python tools/dj_beatport.py login "
              "--paste")
        sys.exit(2)
    return c
